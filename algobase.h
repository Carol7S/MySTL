...
namespace mystl{        //外部接口
    /* fill: 将[first, last) 内的所有位置填充为value */
    template<class ForwardIterator, class T>
    inline void fill(ForwardIterator first, ForwardIterator last, T value){
        for(; first != last; ++first)
            *first = value;
    }

    /* fill_n: 将从first开始的n个位置填充为value，返回最后一个填充元素的下一位置 */
    //注意，OutputIterator是输出（写入），InputIterator是输入（读出），和名字相反
    template<class OutputIterator, class Size, class T>
    inline OutputIterator fill_n(OutputIterator first, Size n, T& value){
        for(; n>0; --n, ++first){
            *first = value;
        }
        return first;
    ...
    //swap
    template<class T>
    inline void swap(const T& lhs, const T& rhs){
        T tmp = lhs;
        lhs = rhs;
        rhs = tmp;
    }
    ...
    namespace detail{
        ...
        template<class T>
        struct __copy_dispatch<const T*, const T*> {	//两个参数都是 const T* 指针形式
            T* operator()(T* first, T* last, T* result) {
                typedef typename __type_traits<T>::has_trivial_assignment_operator has_trivial;
                return __copy_t(first, last, result, has_trivial());
                //根据是否有trivial assignment operator使用不同的策略
    ...
    namespace detail{
        ...
        //反向copy，从最后一个开始copy
        /*其操作示意于图6-4，将[ first,last)区间内的每一个元素，以逆行的方向复制到以result-1为起点，方向亦为逆行的区间上。
        换句话说，copy_backward 算法会执行赋值操作*(result-1) = *(last- 1) , *(result-2) = *(last-2)，…依此类推。
         返回一个迭代器: result-(last-first)。copy_backward所接受的迭代器必须是BidirectionalIterator */
